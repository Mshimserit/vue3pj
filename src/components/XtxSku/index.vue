<template>
  <div class="goods-sku">
    <dl v-for="item in localSpecs" :key="item.id">
      <dt>{{ item.name }}</dt>
      <dd>
...
export default {
  ...
  setup (props, { emit }) {
    ...
    watchEffect(() => {
      // 深拷贝规格数据到本地状态
      localSpecs.value = JSON.parse(JSON.stringify(props.goods.specs))
      // 得到所有字典集合
      pathMap = getPathMap(props.goods.skus)
      // 组件初始化的时候更新禁用状态
      initDisabledStatus(localSpecs.value, pathMap)
    })
    ...
    const clickSpecs = (item, val) => {
      ...
      // 点击之后再次更新选中状态
      updateDisabledStatus(localSpecs.value, pathMap)
      // 把选择的sku信息传出去给父组件
      // 触发change事件将sku数据传递出去
      const selectedArr = getSelectedArr(localSpecs.value).filter(value => value)
      // 如果选中得规格数量和传入得规格总数相等则传出完整信息(都选择了)
      // 否则传出空对象
      if (selectedArr.length === localSpecs.value.length) {
        // 从路径字典中得到skuId
